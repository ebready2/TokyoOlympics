...
function DeleteAthlete() {
  ...
  const submitDelete = () => {
    Axios.post('http://localhost:3002/api/delete', {
      deleteTable: 'Athlete',
      deleteAttribute: 'athleteName',
      deleteValue: deleteValue
    }).then((response) => {
      // display the remaining entries with a similar deleteValue
      setMyQuery(`SELECT * FROM Athlete WHERE athleteName LIKE '%${deleteValue}%';`);
            ...
            <h1 className="response">Your New Country</h1>
                {/* <table>
                    {responseData.map(provider =>
                        <div
                        key={provider.athleteName}> 
                        <div className = "card1">
                            <tr>
                                <td><label>Athlete Name: </label></td>
                                <td><label>{provider.athleteName}</label></td>
                            </tr>
