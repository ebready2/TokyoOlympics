...
function DeleteDiscipline() {
  ...
  const submitDelete = () => {
    Axios.post('http://localhost:3002/api/delete', {
      deleteTable: 'Discipline',
      deleteAttribute: 'discName',
      deleteValue: deleteValue
    }).then((response) => {
      // display the remaining entries with a similar deleteValue
      setMyQuery(`SELECT * FROM Discipline WHERE discName LIKE '%${deleteValue}%';`);
            ...
            <h1 className="response">Your New Country</h1>
                {/* <table>
                    {responseData.map(provider =>
                        <div
                        key={provider.discName}> 
                        <div className = "card1">
                            <tr>
                                <td><label>Discipline Name: </label></td>
                                <td><label>{provider.discName}</label></td>
                            </tr>
